...
{
  ========================================================================
  TDesktopCapture - CURSOR AND VIRTUAL DESKTOP FIXED
  ========================================================================

  FIXED: Mouse cursor now renders as proper white arrow cursor
  FIXED: DXGI virtual desktop capture (auto-falls back to GDI for multi-monitor)
  FIXED: DXGI AccumulatedFrames handling (Thanks to c-michail)
  FIXED: Using proper DXGI dirty region detection instead of manual pixel comparison
}
...
type
  ...
  private
    ...
    // DXGI dirty region tracking
    FLastDirtyRegions: TArray<TDirtyRegion>;
    ...
    function ConvertDXGIDirtyRegions(DirtyRects: PRECT; DirtyCount: UINT): TArray<TDirtyRegion>;
...
begin
  ...
  // Initialize DXGI dirty region tracking
  SetLength(FLastDirtyRegions, 0);
...
{
  FIXED DXGI Frame Capture Function with proper dirty region detection
  ===================================================================

  Now using DXGI's hardware-accelerated GetFrameDirtyRects instead of
  manual pixel comparison. Much more efficient and accurate.
}
...
var
  ...
  // DXGI Dirty Region Detection Variables
  BufLen, BufSize: UINT;
  MetaData: TBytes;
  MoveRects: PDXGI_OUTDUPL_MOVE_RECT;
  DirtyRects: PRECT;
  MoveCount, DirtyCount: UINT;
begin
  SetLength(Result, 0);
  FrameAcquired := False;
  ...
  try
    ...
    // c-michail's critical AccumulatedFrames check
    ...
    // Get DXGI dirty regions and move rectangles (proper way!)
    BufLen := FrameInfo.TotalMetadataBufferSize;
    if BufLen > 0 then
    begin
      SetLength(MetaData, BufLen);

      // Get move rectangles
      MoveRects := PDXGI_OUTDUPL_MOVE_RECT(@MetaData[0]);
      hr := FOutputDuplication.GetFrameMoveRects(BufLen, MoveRects, BufSize);
      if FAILED(hr) then Exit;
      MoveCount := BufSize div SizeOf(TDXGI_OUTDUPL_MOVE_RECT);

      // Get dirty rectangles
      if BufSize < BufLen then
      begin
        DirtyRects := PRECT(@MetaData[BufSize]);
        Dec(BufLen, BufSize);
        hr := FOutputDuplication.GetFrameDirtyRects(BufLen, DirtyRects, BufSize);
        if FAILED(hr) then Exit;
        DirtyCount := BufSize div SizeOf(TRECT);

        // Convert DXGI dirty rects to our format and store them
        FLastDirtyRegions := ConvertDXGIDirtyRegions(DirtyRects, DirtyCount);
        FHasDirtyRegions := (DirtyCount > 0);
      end
      else
      begin
        DirtyCount := 0;
...
function TDesktopCapture.ConvertDXGIDirtyRegions(DirtyRects: PRECT; DirtyCount: UINT): TArray<TDirtyRegion>;
var
  i: Integer;
  CurrentRect: PRECT;
begin
  SetLength(Result, DirtyCount);
  CurrentRect := DirtyRects;

  for i := 0 to Integer(DirtyCount) - 1 do
  begin
    Result[i].Left := CurrentRect.Left;
    Result[i].Top := CurrentRect.Top;
    Result[i].Right := CurrentRect.Right;
    Result[i].Bottom := CurrentRect.Bottom;
    Inc(CurrentRect);
  end;
end;
    ...
    try
      ...
      case FOwner.FMode of
        ...
        begin
          ...
          begin
            // Use DXGI dirty regions if available (from DXGI capture), otherwise fallback to manual detection
            if (FOwner.FMethod = cmDXGI) and FOwner.FHasDirtyRegions then
            begin
              DirtyRegions := Copy(FOwner.FLastDirtyRegions);
            ...
            begin
              // Fallback to manual detection for GDI or when DXGI dirty regions not available
