...
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Result[RegionCount - 1].Left := x * BLOCK_SIZE;
            Result[RegionCount - 1].Top := y * BLOCK_SIZE;
            Result[RegionCount - 1].Right := Min((x + 1) * BLOCK_SIZE, Width);
            Result[RegionCount - 1].Bottom := Min((y + 1) * BLOCK_SIZE, Height);
            Break;
          end;
        end;
      end;
  ...
  // If too many regions, just do full frame
  if Length(Result) > (Width * Height) div (BLOCK_SIZE * BLOCK_SIZE * 4) then
  begin
    SetLength(Result, 1);
    Result[0].Left := 0;
    Result[0].Top := 0;
...
// FIXED: Much more robust cursor drawing
...
begin
  ...
  try
    ...
    // Convert to capture area coordinates
